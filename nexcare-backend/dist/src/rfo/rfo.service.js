...
exports.RFOService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const rfo_entity_1 = require("./rfo.entity");
const puppeteer = __importStar(require("puppeteer"));
let RFOService = class RFOService {
    rfoRepository;
    constructor(rfoRepository) {
        ...
        if (search) {
            return this.rfoRepository.find({
                where: [
                    ...
                    { rfoNumber: (0, typeorm_2.Like)(`%${search}%`) },
                    { cidIw: (0, typeorm_2.Like)(`%${search}%`) },
                ],
    ...
    async findById(id) {
        ...
        if (!rfo)
            throw new common_1.NotFoundException('RFO not found');
        return rfo;
    }
    async create(dto, userId, userName) {
        const rfo = this.rfoRepository.create({
            ...
            status: rfo_entity_1.RFOStatus.PENDING,
        });
        return this.rfoRepository.save(rfo);
    }
    async update(id, dto) {
        const rfo = await this.findById(id);
        if (rfo.status === rfo_entity_1.RFOStatus.APPROVED) {
            throw new common_1.ForbiddenException('Cannot edit an approved RFO');
        }
    ...
    async approve(id, userId) {
        ...
        await this.rfoRepository.update(id, {
            status: rfo_entity_1.RFOStatus.APPROVED,
            approvedById: userId,
            approvedAt: new Date(),
        });
        return this.findById(id);
...
exports.RFOService = RFOService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(rfo_entity_1.RFO)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], RFOService);
//# sourceMappingURL=rfo.service.js.map